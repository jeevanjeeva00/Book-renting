...
const rentController = {
    ...
    create: async(req,res)=>{
        try{
            let extRent = await Rent.findOne({bookId :  req.body.bookId}&& {userId: req.body.userId})
                if(extRent)
                    return res.status(400).json({ msg: `Already you have a rented book...`})

           const data = await Rent.create(req.body)
                return res.status(200).json({ msg: `Rent details addded succesfully` , rent : data})
        }catch(err){
            return res.status(500).json({ msg: err.message})
        }
    },
        ...
        try{
            ...
            let extRent = await Rent.findById({ _id: id})
                ...
                await Rent.findByIdAndUpdate({ _id : id} ,{
                    ...
                    return res.status(500).json({ msg: `Rent details Updated Successfuly`})
        }catch(err){
            return res.status(500).json({ msg: err.message})
        }
    },
    delete: async(req,res)=>{
        try{
            ...
            let extRent = await Rent.findById({_id : id})
                if(!extRent)
                    return res.status(404).json({ msg : `Requested  Rent if not found`})

                await Rent.findByIdAndDelete({_id : id})
                    return res.status(200).json({msg: `Rent details deleted Succesfully.`})
